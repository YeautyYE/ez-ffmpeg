...
| `-c:v:0`, `-b:a:1`, ... (per-stream indexed) | typed | [`Output::add_stream_map`](crate::Output::add_stream_map)`(`[`StreamMap`](crate::StreamMap)`::new("0:v:0").codec(...).codec_opt(...))` | Per-map encoder and options for exactly the mapped streams; per-map keys override the per-type `set_*_codec`/`set_*_codec_opt` values key by key. *cli facade*: indexed per-stream forms are rejected. |
...
| `-progress` / `-stats` | typed | [`FfmpegScheduler`](crate::FfmpegScheduler)`::progress_handle()` → [`ProgressHandle`](crate::ProgressHandle)`::snapshot()` | Typed per-output progress (the CLI's `frame`/`fps`/`out_time`/`total_size`/`bitrate`/`speed` fields) via pull-based snapshots; the textual `-progress` key/value interface itself is not emitted. A `FrameFilter` observing decoded-frame timestamps (`examples/processing_progress`) remains an alternative on the decode side. |
