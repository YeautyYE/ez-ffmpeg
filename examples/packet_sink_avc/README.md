...
1. Use an FFmpeg build that includes libx264; the example prints a notice and
   exits otherwise. (The strict tier's verified whitelist also admits
   `h264_nvenc`, `h264_videotoolbox`, and `libopenh264` — swap the
   `set_video_codec` name to use one of those.)
