...
- **`FrameExtractor`** turns a video into owned, tightly packed 8-bit pixel
  ...
  and time windows. The resize + pixel-format conversion defaults to the
  FFmpeg CLI's own swscale configuration — the same bytes as an equivalent
  ...
  behavior; `High` reproduces v0.14.0 output — byte-identical when linked
  against the same libswscale build — while the current default matches the
  CLI.)
