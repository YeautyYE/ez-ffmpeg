...
use ffmpeg_sys_next::{
    AV_NOPTS_VALUE, AV_PKT_FLAG_CORRUPT, AV_TIME_BASE, AV_TIME_BASE_Q, AVCodecDescriptor,
    AVCodecParameters, AVERROR, AVERROR_EOF, AVFMT_TS_DISCONT, AVFormatContext, AVMediaType,
    AVPacket, AVRational, AVStream, EAGAIN, av_compare_ts, av_gettime_relative, av_inv_q, av_mul_q,
    av_packet_ref, av_q2d, av_read_frame, av_rescale, av_rescale_q, av_rescale_q_rnd,
    av_stream_get_parser, av_usleep, avformat_seek_file,
};
    ...
    unsafe {
        ...
        for i in 0..nb_streams {
            ...
            if let Some(ds) = option {
                ...
                let now = ((av_gettime_relative() - demux_paramter.wallclock_start) as f32
                    * readrate) as i64
                    + stream_ts_offset;
    ...
    unsafe {
        ...
        if !ds.saw_first_ts {
            ds.dts = if (*ist).avg_frame_rate.num != 0 {
                (((-(*par).video_delay) * AV_TIME_BASE) as f64 / av_q2d((*ist).avg_frame_rate))
                    as i64
            } else {
                0
            };
...
unsafe fn seek_to_start(
    ...
    unsafe {
        ...
        if demux_paramter.end_pts.ts != AV_NOPTS_VALUE
            && demux_paramter.max_pts.ts == AV_NOPTS_VALUE
            || av_compare_ts(
                demux_paramter.max_pts.ts,
                demux_paramter.max_pts.tb,
        ...
        if independent_readrate && wait_time != 0 {
            if let Some(readrate) = demux_paramter.readrate {
                if readrate != 0.0 {
                    ...
                    debug!(
                        "FFmpeg on-demand scheduling caused the initial wallclock_start to not meet the specified readrate:{readrate}. Adjusting wallclock_start from {} to {fix_wallclock_start}",
