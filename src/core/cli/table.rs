...
pub(crate) const KNOWN_REJECTIONS: &[(&str, &str, Option<&str>)] = &[
    ...
    ("-af", "audio filters are outside the verified CLI subset (the builder equivalent exists: Output::set_audio_filter)", None),
    ("-filter:a", "audio filters are outside the verified CLI subset (the builder equivalent exists: Output::set_audio_filter)", None),
    ...
    ("-progress", "the textual progress interface is outside the subset (the builder equivalent exists: FfmpegScheduler::progress_handle)", None),
    ("-stats_period", "the textual progress interface is outside the subset (the builder equivalent exists: FfmpegScheduler::progress_handle)", None),
    ("-t:v", "per-stream duration is permanently excluded: the crate models recording time per output, not per stream", None),
