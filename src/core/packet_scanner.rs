use ffmpeg_sys_next::{
    av_packet_alloc, av_packet_free, av_packet_unref, av_read_frame, avformat_seek_file, AVPacket,
    AVERROR, AV_PKT_FLAG_CORRUPT, AV_PKT_FLAG_KEY, EAGAIN,
};
...
impl PacketScanner {
    ...
    pub fn seek(&mut self, timestamp_us: i64) -> Result<()> {
        ...
        unsafe {
            ...
            if ret < 0 {
                return Err(PacketScannerError::SeekError(DemuxingError::from(ret)).into());
            }
        }
        Ok(())
    }
            ...
            loop {
                ...
                if ret == AVERROR(EAGAIN) {
                    ...
                    if eagain_retries > MAX_EAGAIN_RETRIES {
                        return Err(PacketScannerError::ReadError(DemuxingError::from(ret)).into());
                    }
